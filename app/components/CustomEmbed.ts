...
import type { Card, HighlightElement, MediaCard } from "~/types";
...
export default Node.create({
  ...
  renderHTML({ HTMLAttributes, node }) {
    ...
    const highlight = highlightData.find((h: HighlightElement) => h.id === node.attrs.objectId);
    const card = cards.find((c: Card) => c.id === node.attrs.objectId);

    const mediaCard = allMediaCards.find((m: MediaCard) => m.id === node.attrs.objectId);
    ...
    const childNodesCard = Array.from(docCard.body.children).map((child) => parseNode(child));

    const childNodes = Array.from(doc.body.children).map((child) => parseNode(child));

    const childNodesNote = Array.from(docNote.body.children).map((child) => parseNode(child));
    ...
    if (highlight) {
      return [
        ...
        [
          ...
          isNoteEmpty
            ? ["div"]
            : ["div", { class: "pt-[0.5rem] px-[1.75rem] pb-0" }, ...childNodesNote],
        ],
      ];
    }
    ...
    if (card) {
      return [
        ...
        [
          "div",
          {
            class: "py-6 px-7 bg-ultra-light-grey border-light-grey border border-solid rounded",
          },
          ...childNodesCard,
        ],
      ];
        ...
        mergeAttributes(
          {
            ...
            class: "transition-[width] duration-300 ease-out w-full my-2 self-center",
            style: "width: 100%;",
          },
          HTMLAttributes,
        ),
