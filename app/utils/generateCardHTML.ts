...
import type { Card, HighlightElement, MediaCard, MentionInfo, TiptapNode } from "~/types";
...
export function generateCardHTML({
  ...
  const transformedContent = transformListItems(content).map((item: TiptapNode) =>
    ...
    ? generateHTML(
        ...
        [
          ...
          CodeBlock.extend({
            ...
            renderHTML({ HTMLAttributes }: { HTMLAttributes: { params: string } }) {
